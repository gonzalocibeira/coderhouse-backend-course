...
const getMain = async (req, res) => {
    ...
    res.render("main", {username: req.user.username, address: req.user.address, pic: req.user.pic, name: req.user.name, age: req.user.age, phone: req.user.phone, products: prods})
};
...
const postMain = async (req, res) => {
    ...
    const products = ProductFactory.createDAO("mongo");
    const prods = await products.getProducts();
    res.render("main", {username: req.body.username, address: req.user.address, pic: req.user.pic, name: req.user.name, age: req.user.age, phone: req.user.phone, products: prods})
};
...
const getProductById = async (req, res) => {
    const products = ProductFactory.createDAO("mongo");
    const product = await products.getProdById(req.params.id);
    res.status(200).json(product)
};

const getProductByCategory = async (req, res) => {
    const products = ProductFactory.createDAO("mongo");
    const product = await products.getProdByCategory(req.params.category);
    res.status(200).json(product)
};
...
export { getLogin, getRegister, postRegister, getRegisterFail, isAuthenticated, getMain, postMain, getLoginFail, getLogout, fakerData, postAdd, getCart, deleteCart, postBuy, getSysSpecs, getProducts, getProductById, getProductByCategory, getChatsByEmail, unkownRoute };